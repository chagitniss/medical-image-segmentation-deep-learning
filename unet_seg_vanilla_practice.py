...
def load_image(path_to_image):
    ...
    """
    # # --------  read the image in image_path to img_array (read docs for skimage.io.imread), with 'float' data type (using 'astype')
    img_array = imread(path_to_image)
    # # -------- convert img_array from rgba to rgb
...
def load_mask(path_to_mask):
    ...
    """
    # # -------- read the mask in path_to_mask to mask_array as a grayscale image
...
def preprocess_image(img_array):
    ...
    :return: a processed version of the input image.
             Currently implemented: a resize of the input image to (image_height_for_training, image_width_for_training, 3).
    """
    # # -------- Resize img_array to (image_height_for_training, image_width_for_training, 3). use the argument order=3
...
def preprocess_mask(mask_array):
    ...
    :return: a processed version of the input maske.
             Currently implemented: a resize of the input image to (image_height_for_training, image_width_for_training).
    """
    # # -------- Resize mask_array to (image_height_for_training, image_width_for_training). use the argument order=1
    processed_mask = resize(mask_array,(image_height_for_training,image_width_for_training), order=3)
    #
    ...
    while True:
        ...
        for batch_index in range(0, num_files - batch_size + 1, batch_size):
            for b in range(0, batch_size):
                ...
                # -------- Add a dimension to both img_array and mask_array so that they can be concatenated
                # as batches along the first axis of the tensor ( use np.expand_dims(.) ):
                img_array = np.expand_dims(img_array, axis=0)  # write your implementation
                mask_array = np.expand_dims(mask_array, axis=0)  # write your implementation
...
def convolution_block(input_layer, num_filters, kernel_size, activation_func):
    """
    # ------ Implement a function that creates two convolution layers, both with the arguments input_layer,
    num_filters, kernel_size, activation_func. Use 'same' padding and 'he_uniform' kernel_initializer.
    The first layer gets input_layer as argument and the second layer gets the first layer as argument.
    See the keras documentation of Conv2D(.) in: https://keras.io/layers/convolutional/
    """
...
def conv_downsample_block(input_layer, num_filters, kernel_size, activation_func, max_pool_shape):
    """
    # ------ Implement a function that creates a down-sampling block: and one MaxPooling2D layer and one convolution
    layer. Use 'same' padding for the convolution and pool_size=max_pool_shape for the pooling layer.
    The first layer gets input_layer as argument and the second layer gets the first layer as argument.
    See the keras documentation of MaxPooling2D(.) in: https://keras.io/layers/pooling/
    """
    pool = MaxPooling2D(pool_size=max_pool_shape, padding='same')(input_layer)
...
def conv_upsample_block(input_layer, skip_connection_layer, num_filters, kernel_size, activation_func,
                        upsampling_shape):
    """
    # ------ Implement a function that creates a up-sampling block:
    one UpSampling2D layer, one concatenation layer and one convolution layer.
    Use 'same' padding for the convolution and size=upsampling_shape for the up-sampling layer.
    The first layer gets input_layer as argument.
    The second layer should concatenate the upsampled input (given by the first layer, upsampling) and the
    skip_connection_layer received as input parameter. Use axis=3 as concatenation parameter.
    The last layer should receive the concatenated layers as input.
    See the keras documentation of UpSampling2D(.) in: https://keras.io/layers/convolutional/
    and of concatenate in: https://www.tensorflow.org/api_docs/python/tf/keras/layers/concatenate
    """
...
def get_unet(img_rows, img_cols, first_layer_num_filters=32, num_classes=1):
    ...
    ###### DECODER BRANCH ######input_layer, skip_connection_layer,
...
def create_and_train_model(training_data, validation_data, input_height, input_width, first_layer_num_filters,
    ...
    model_checkpoint = ModelCheckpoint('cell_unet_weights_{epoch:02d}-{val_loss:.2f}-{val_acc:.2f}.h5',
                                       monitor='val_loss',
    ...
    model.save('cell_unet_weights.h5')
##############################################################################################################
